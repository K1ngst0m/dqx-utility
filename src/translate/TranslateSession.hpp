...
public:
    ...
    SubmitResult submit(const std::string& processed_text, TranslationConfig::TranslationBackend backend,
                        TranslationConfig::TargetLang target, translate::ITranslator* translator)
    {
        ...
        key += "B:";
        key += std::to_string(static_cast<int>(backend));
        key += "|T:";
        key += target_code;
        key += "|";
        key += processed_text;
        ...
        {
            utils::ErrorReporter::ReportWarning(utils::ErrorCategory::Translation, "Translator backend is not ready",
    ...
    void onCompleted(const std::vector<translate::Completed>& results, std::vector<CompletedEvent>& out_events)
    {
        out_events.clear();
