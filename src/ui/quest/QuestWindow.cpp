...
QuestWindow::QuestWindow(FontManager& font_manager, GlobalStateManager& global_state, ConfigManager& config, QuestManager& quest_manager, MonsterManager& monster_manager, int instance_id, const std::string& name, bool is_default)
    ...
    , name_(name)
{
    ...
    drawer_helper_ = std::make_unique<QuestHelperWindow>(font_manager_, global_state_, config_, quest_manager_, monster_manager_, drawer_name);
...
void QuestWindow::refreshFontBinding() { font_manager_.ensureFont(state_.ui_state()); }
...
void QuestWindow::renderSettings() { show_settings_window_ = true; }
...
void QuestWindow::renderQuestContent(float wrap_width)
{
    ...
    if (!subquest_text.empty())
    {
        ...
        ui::RenderAnnotatedText(subquest_text.c_str(), sub_pos, ImGui::GetFont(), ImGui::GetFontSize(), wrap_width, &monster_manager_);
    ...
    else
    {
        ...
        const std::string fallback =
            ui::LocalizedOrFallback("quest.description.empty", "No quest description available");
        ImVec2 desc_pos = ImGui::GetCursorScreenPos();
        desc_pos.x += description_side_margin;
...
void QuestWindow::renderTranslationControls(float wrap_width)
{
    ...
    {
        ImGui::TextDisabled(
            "%s", ui::LocalizedOrFallback("quest.translation.disabled", "Translation disabled. Enable it in settings.")
                      .c_str());
        ImGui::PopTextWrapPos();
        return;
    }
    ...
    const bool has_failure = std::any_of(field_status_.begin(), field_status_.end(),
                                         [](const FieldStatus& s)
                                         {
                                             return s.failed;
                                         });
    ...
    if (translatorConfigIncomplete(cfg, incomplete_reason))
    {
        if (!translator_error_reported_)
        {
            utils::ErrorReporter::ReportError(utils::ErrorCategory::Translation, utils::ErrorSeverity::Info,
                                              "Quest translator disabled: configuration incomplete", incomplete_reason);
    ...
    if (!translator_ || !translator_->init(cfg))
    {
        ...
        if (!translator_error_reported_)
        {
            utils::ErrorReporter::ReportError(
                utils::ErrorCategory::Translation, utils::ErrorSeverity::Warning,
...
void QuestWindow::processTranslatorEvents()
{
    ...
    for (const auto& ev : events)
    {
        ...
        if (ev.failed)
        {
            handleTranslationFailure(field,
...
void QuestWindow::submitFieldTranslation(QuestField field, const std::string& text, const TranslationConfig& config)
{
    ...
    handleTranslationFailure(
        field, ui::LocalizedOrFallback("quest.translation.queue_failed", "Unable to queue translation request."));
}
...
void QuestWindow::applyCachedTranslation(QuestField field, const std::string& text)
{
    ...
    switch (field)
    {
    case QuestField::SubQuest:
        state_.translated.subquest_name = text;
        break;
    case QuestField::Title:
        state_.translated.quest_name = text;
        break;
    case QuestField::Description:
        state_.translated.description = text;
        break;
    case QuestField::Rewards:
        state_.translated.rewards = text;
        break;
    case QuestField::RepeatRewards:
...
std::string QuestWindow::displayStringFor(QuestField field) const
{
    ...
    if (status.has_translation)
    {
        ...
        {
        case QuestField::SubQuest:
            return state_.translated.subquest_name;
        case QuestField::Title:
            return state_.translated.quest_name;
        case QuestField::Description:
            return state_.translated.description;
        case QuestField::Rewards:
            return state_.translated.rewards;
        case QuestField::RepeatRewards:
            return state_.translated.repeat_rewards;
        }
    }
    ...
    {
    case QuestField::SubQuest:
        return state_.quest.subquest_name;
    case QuestField::Title:
        return state_.quest.quest_name;
    case QuestField::Description:
        return state_.quest.description;
    case QuestField::Rewards:
        return state_.quest.rewards;
    case QuestField::RepeatRewards:
        return state_.quest.repeat_rewards;
    }
    return {};
}

QuestWindow::FieldStatus& QuestWindow::fieldStatus(QuestField field) { return field_status_[fieldIndex(field)]; }
...
std::size_t QuestWindow::fieldIndex(QuestField field) { return static_cast<std::size_t>(field); }
...
void QuestWindow::renderDrawerHelper()
{
    ...
    bool width_changed = std::abs(drawer_helper_->state().ui.width - quest_size.x) > 1.0f;
    if (width_changed)
    {
        drawer_helper_->state().ui.pending_resize = true;
    }
    ...
    if (window_open)
    {
        ...
        if (drawer_helper_->state().ui.pending_resize && 
            std::abs(win_size.x - quest_size.x) < 2.0f &&
