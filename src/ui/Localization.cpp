...
void flatten_table(const toml::table& tbl, const std::string& prefix, std::unordered_map<std::string, std::string>& out)
{
    for (auto&& [k, node] : tbl)
    {
...
std::unordered_map<std::string, std::string> load_file(const fs::path& p)
{
    ...
    if (!fs::exists(p, ec))
    {
        PLOG_WARNING << "i18n file not found: " << p.string();
        {
            ...
            {
                utils::ErrorReporter::ReportWarning(utils::ErrorCategory::Configuration, "Missing localization file",
                                                    p.string());
            }
        }
        return r;
        ...
        PLOG_WARNING << "Failed to parse i18n file '" << p.string() << "': " << pe.description();
        {
            ...
            if (s_parse_failures.insert(p.string()).second)
            {
                utils::ErrorReporter::ReportWarning(utils::ErrorCategory::Configuration,
                                                    "Failed to parse localization file", std::string(pe.description()));
            }
        }
    }
    return r;
...
const std::string& current_language() { return s_lang; }
...
const char* get(const char* key) { return get_str(std::string(key)).c_str(); }
