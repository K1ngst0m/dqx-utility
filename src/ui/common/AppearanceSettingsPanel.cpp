...
AppearanceSettingsPanel::RenderResult AppearanceSettingsPanel::render()
{
    ...
    result.width_changed =
        ImGui::SliderFloat("##dialog_width_slider", &state_.ui_state().width, 200.0f, max_dialog_width);
    ImGui::Spacing();

    ImGui::TextUnformatted(i18n::get("dialog.appearance.height"));
    set_slider_width();
    result.height_changed =
        ImGui::SliderFloat("##dialog_height_slider", &state_.ui_state().height, 80.0f, max_dialog_height);
    ImGui::Spacing();
    ...
    set_slider_width();
    result.alpha_changed =
        ImGui::SliderFloat("##dialog_bg_alpha_slider", &state_.ui_state().background_alpha, 0.0f, 1.0f);
    ImGui::Spacing();
    ...
    result.font_changed =
        ImGui::SliderFloat("##dialog_font_size_slider", &state_.ui_state().font_size, min_font, max_font);
    ImGui::Spacing();

    ImGui::Separator();
    ImGui::Spacing();
    ImGui::TextUnformatted(i18n::get("dialog.appearance.fade.label"));
    bool fade_enabled_changed =
