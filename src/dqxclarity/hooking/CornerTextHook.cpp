...
CornerTextHook::CornerTextHook(const HookCreateInfo& create_info)
    : HookBase(create_info)
{
}

Pattern CornerTextHook::GetSignature() const
{
    return Signatures::GetCornerText();
}

std::vector<uint8_t> CornerTextHook::GenerateDetourPayload()
{
...
bool CornerTextHook::PollCornerText()
{
    ...
    if (!memory()->ReadMemory(backup_address(), &text_ptr_raw, sizeof(text_ptr_raw)))
    {
        uint8_t zero = 0;
    ...
    std::string text;
    if (!memory()->ReadString(text_ptr, text, kMaxStringLength))
    {
        text.clear();
    }
