...
bool QuestHook::InstallHook(bool enable_patch) {
    ...
    if (!FindQuestTriggerAddress()) {
        if (m_logger.error) m_logger.error("Failed to find quest trigger address");
        return false;
    }

    if (!AllocateDetourMemory()) {
        if (m_logger.error) m_logger.error("Failed to allocate quest detour memory");
        return false;
    }
    ...
    if (!m_memory->ReadMemory(m_hook_address, m_original_bytes.data(), stolen_bytes)) {
        if (m_logger.error) m_logger.error("Failed to read quest hook original bytes");
        return false;
    }

    if (!WriteDetourCode()) {
        if (m_logger.error) m_logger.error("Failed to write quest detour code");
        return false;
    }
...
bool QuestHook::EnablePatch() {
    ...
    if (!MemoryPatch::WriteWithProtect(*m_memory, m_hook_address, patch_bytes)) {
        if (m_logger.error) m_logger.error("Failed to write quest hook patch bytes");
        return false;
    }
...
bool QuestHook::ReapplyPatch() {
    ...
    if (!MemoryPatch::WriteWithProtect(*m_memory, m_hook_address, patch_bytes)) {
        if (m_logger.error) m_logger.error("Failed to reapply quest hook patch");
        return false;
    }
...
bool QuestHook::PatchOriginalFunction() {
    return EnablePatch();
}
