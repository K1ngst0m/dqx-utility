...
QuestHook::QuestHook(const HookCreateInfo& create_info)
    : HookBase(create_info)
{
}

Pattern QuestHook::GetSignature() const
{
    return Signatures::GetQuestText();
}

std::vector<uint8_t> QuestHook::GenerateDetourPayload()
{
...
bool QuestHook::PollQuestData()
{
    ...
    if (!memory()->ReadMemory(backup_address(), &quest_ptr_raw, sizeof(quest_ptr_raw)))
    {
        uint8_t zero = 0;
    ...
    {
        std::string value;
        if (memory()->ReadString(quest_ptr + offset, value, kMaxStringLength))
        {
            out = std::move(value);
        }
        else
