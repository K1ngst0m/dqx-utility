...
{
    return m_is_attached ? static_cast<pid_t>(m_process_id) : static_cast<pid_t>(-1);
}
...
bool ProcessMemory::SetMemoryProtection(uintptr_t address, size_t size, MemoryProtectionFlags protection)
{
    ...
    return VirtualProtectEx(m_process_handle, reinterpret_cast<LPVOID>(address), size, win_protection,
                            &old_protection) != 0;
}
...
uintptr_t ProcessMemory::GetModuleBaseAddress(const std::string& module_name)
{
    ...
    if (!EnumProcessModules(m_process_handle, modules, sizeof(modules), &bytes_needed))
        return 0;
...
{
    if (!m_is_attached || offsets.empty())
        return 0;
    uintptr_t current = ReadInt32(base);
    for (size_t i = 0; i < offsets.size(); ++i)
    {
        if (i != offsets.size() - 1)
            current = ReadInt32(current + offsets[i]);
        else
...
DWORD ProcessMemory::ConvertProtectionFlags(MemoryProtectionFlags flags)
{
    switch (flags)
    {
    case MemoryProtectionFlags::Read:
        return PAGE_READONLY;
    case MemoryProtectionFlags::Write:
        return PAGE_READWRITE;
    case MemoryProtectionFlags::Execute:
        return PAGE_EXECUTE;
    case MemoryProtectionFlags::ReadWrite:
        return PAGE_READWRITE;
    case MemoryProtectionFlags::ReadExecute:
        return PAGE_EXECUTE_READ;
    case MemoryProtectionFlags::ReadWriteExecute:
