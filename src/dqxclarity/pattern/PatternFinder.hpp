...
public:
    ...
    std::vector<uintptr_t> FindAllInModuleNaive(const Pattern& pattern, const std::string& module_name) {
        ...
        if (!memory_ || !memory_->IsProcessAttached()) return results;
        ...
        for (const auto& r : regions) {
            ...
            if (path.find(mod) == std::string::npos) continue;
            if (!r.IsReadable()) continue;
            size_t size = r.Size();
            if (size < pattern.Size()) continue;
            std::vector<uint8_t> buf(size);
            if (!memory_->ReadMemory(r.start, buf.data(), buf.size())) continue;
            for (size_t i = 0; i + pattern.Size() <= buf.size(); ++i) {
                if (MatchAt(buf, i, pattern)) results.push_back(r.start + i);
            }
        }
        return results;
    }
