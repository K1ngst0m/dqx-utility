...
public:
    ...
    std::optional<uintptr_t> ScanRegion(
        const MemoryRegion& region,
        const Pattern& pattern
    );

    std::vector<uintptr_t> ScanRegionAll(
        const MemoryRegion& region,
        const Pattern& pattern
    );

    std::optional<uintptr_t> ScanProcess(
        const Pattern& pattern,
        bool require_executable = true
    );

    std::optional<uintptr_t> ScanModule(
        const Pattern& pattern,
        const std::string& module_name
    );

    std::vector<uintptr_t> ScanProcessAll(
        const Pattern& pattern,
        bool require_executable = true
    );
...
private:
    ...
    std::optional<size_t> FindPatternInBuffer(
    ...
    std::vector<size_t> FindPatternInBufferAll(
        const uint8_t* buffer,
        size_t buffer_size,
        const Pattern& pattern
    );
