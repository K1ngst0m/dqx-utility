...
void report_wine_warning(std::unordered_set<std::string>& bucket, const std::string& key, const char* user_message,
...
std::optional<WineEnvironment> WineDetector::detectWineEnvironment(const std::string& processName)
{
    ...
    if (wine_env.wine_prefix.empty())
    {
        ...
        report_wine_warning(g_no_wine_reported, processName, "Wine environment not detected", processName);
        return std::nullopt;
    }
    ...
    {
        report_wine_warning(g_invalid_prefix_reported, wine_env.wine_prefix, "Wine prefix validation failed",
...
std::optional<int> WineDetector::findSingleProcessByName(const std::string& processName)
{
    ...
    if (matching_pids.empty())
    {
        ...
        report_wine_warning(g_no_process_reported, processName, "Wine process not found", processName);
        return std::nullopt;
    }

    if (matching_pids.size() > 1)
    {
        PLOG_ERROR << "Multiple processes found matching " << processName << " (count: " << matching_pids.size()
                   << "). Wine detection requires single instance.";
        report_wine_warning(g_multiple_process_reported, processName, "Multiple game processes detected",
...
std::unordered_map<std::string, std::string> WineDetector::readProcessEnvironment(int pid)
{
    ...
    {
        report_wine_warning(g_env_fail_reported, pid, "Failed to read process environment", environ_path.string());
        return env;
    }
...
bool WineDetector::validateWinePrefix(const std::string& prefix)
{
    ...
    std::vector<std::string> wine_markers = { "system.reg", "user.reg", "drive_c", "dosdevices" };
